...
def plotar_retornos(ticker: str, df: pd.DataFrame):
    ...
    plt.tight_layout()
    fig.savefig(f"graficos/{ticker}.png")
...
def backtest(ticker: str):
    df = pd.read_parquet(f"data/strategies/{ticker}.parquet")
    ...
    print(f"Valor inicial: {balance}. Buy and hold: {buy_n_hold}")
    ...
    df.to_parquet(f"data/strategies/{ticker}-resultado.parquet")
...
if __name__ == "__main__":
    ...
    ticker = args.ticker
    if not ticker:
        tickers = os.listdir("data/strategies")
